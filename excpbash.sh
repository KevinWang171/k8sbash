...
do
    case $arg in
        -c|--compress)
        echo 'you have selected -c flag, so [file/folder] will be compressed'
        compress=true
        ...
        echo "-c|--compress -> use it when you want to compress file which you are going to download"
        exit 1
        ;;        
        *)
        OTHER_ARGUMENTS=("$1 $2")
...
#检查文件是否存在
	...
	folder_check=$(kubectl exec $pod_name -- sh -c "test -d $folder_for_cp && echo 'it exists' || echo 'it does not exists'")
	file_check=$(kubectl exec $pod_name -- sh -c "test -f $folder_for_cp && echo 'it exists' || echo 'it does not exists'")
	...
	if [ "$compress" = true ]
		then
			...
			kubectl exec $pod_name -- sh -c "cd $cp_dir && tar -czvf /tmp/$arch_file $cp_file"
			echo "${cp_file} has been compressed"
			kubectl exec $pod_name -- sh -c "ls -lstrh /tmp | grep $arch_file"
	fi
	
	echo "we will download ${cp_file}"
	if [ "$compress" = true ]
		then
			kubectl cp "$pod_name:/tmp/$arch_file" "$current_folder/$arch_file"
			ls -lstrh | grep "$arch_file"
			
			echo "clear files on k8s side"
			kubectl exec $pod_name -- sh -c "rm /tmp/$arch_file"
	else
		kubectl cp "$pod_name:$cp_dir/$cp_file" "$current_folder/$cp_file"
